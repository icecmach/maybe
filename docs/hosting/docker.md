# Self Hosting Aurabok with Docker

This guide will help you setup, update, and maintain your self-hosted Aurabok application with Docker Compose. Docker Compose is the most popular and recommended way to self-host the Aurabok app.
...
```bash
# Download the sample compose.yml file from the Aurabok Github repository
curl -o compose.yml https://raw.githubusercontent.com/icecmach/aurabok/main/compose.example.yml
```
...
If everything is working, you will see the Aurabok login screen.
...
Most self-hosting users will want the Aurabok app to run in the background on their computer so they can access it at all times. To do this, hit `Ctrl+C` to stop the running process, and then run the following command:
...
If you find bugs or have a feature request, be sure to read through our [contributing guide here](https://github.com/icecmach/aurabok/wiki/How-to-Contribute-Effectively-to-this-Project).

## How to update your app

The mechanism that updates your self-hosted Aurabok app is the GHCR (Github Container Registry) Docker image that you see in the `compose.yml` file:

```yml
image: ghcr.io/icecmach/aurabok:latest
```

We recommend using one of the following images, but you can pin your app to whatever version you'd like (see [packages](https://github.com/icecmach/aurabok/pkgs/container/aurabok)):

- `ghcr.io/icecmach/aurabok:latest` (latest commit)
- `ghcr.io/icecmach/aurabok:stable` (latest release)
...
```bash
cd ~/docker-apps/aurabok # Navigate to whatever directory you configured the app in
...
```yml
image: ghcr.io/icecmach/aurabok:stable
```
...
If you are trying to get Aurabok started for the **first time** and run into database connection issues, it is likely because Docker has already initialized the Postgres database with a _different_ default role (usually from a previous attempt to start the app).

If you run into this issue, you can optionally **reset the database**.

> [!IMPORTANT]
> **PLEASE NOTE: this will delete any existing data that you have in your Aurabok database, so proceed with caution.** For first-time users of the app just trying to get started, you're generally safe to run the commands below.

By running the commands below, you will delete your existing Aurabok database and "reset" it.

```
docker compose down
docker volume rm aurabok_postgres-data # this is the name of the volume the DB is mounted to
docker compose up
docker exec -it aurabok-postgres-1 psql -U aurabok -d aurabok_production -c "SELECT 1;" # This will verify that the issue is fixed
```
